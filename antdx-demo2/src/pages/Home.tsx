...
const placeholderPromptsItems = [
  {
    ...
    children: [
      {
        key: "1-1",
        description: `如何注册商标并确保我的商标权益得到全面的法律保护？`,
      },
      {
        key: "1-2",
        description: `如何判断一份合同是否有效？在合同履行过程中，如果双方发生纠纷，应如何处理？`,
      },
      {
        key: "1-3",
        description: `如果他人的民事权益受到侵害，但行为人认为自己没有过失，那么责任归属应如何确定？`,
      },
    ],
  },
];
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchConversations() {
      ...
      try {
        ...
        console.log(
          `${process.env.REACT_APP_API_URL}/v1/conversations?user=${userId}`
        );
  ...
  const onConversationClick = async (key: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/v1/messages?conversation_id=${conversationId}&user=${userId}&limit=20`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${apiKey}`,
